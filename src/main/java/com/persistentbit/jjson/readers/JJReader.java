...
public class JJReader
{
    ...
    public <T>T read(JJNode node,Class<T> cls){
    ...
    public void addGeneralReader(Class<?> cls, JJObjectReader reader){
        generalReaders.put(cls,reader);
    }

    public void addReader(Class<?> cls, JJObjectReader reader){
    ...
    {
        Class<JJReader> cls = JJReader.class;

        Method m = cls.getDeclaredMethod("test");
        Type t = (Type)m.getGenericReturnType();
        ...
        System.out.println(ppt.getRawType());
